...
class Api extends BaseController
{
	...
	function feedsListing()
	{
		...
		echo json_encode(array(
			"status" => "ok", "feeds" => $this->rssfeeds_model->data, "date" => $this->rssfeeds_model->date
		));
	}
	...
	function videosListing()
	{
		...
		echo json_encode(array(
			"status" => "ok", "videos" => $this->youtubefeeds_model->data, "date" => $this->youtubefeeds_model->date
		));
	}
	...
	public function update_post_total_views()
	{
		$data = $this->get_data();
		if (!empty($data)) {
			$post_id = isset($data->post_id) ? filter_var($data->post_id, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : "";
			$feed_type = isset($data->feed_type) ? filter_var($data->feed_type, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : "";
	...
	public function gettotallikesandcommentsviews()
	{
		...
		echo json_encode(array(
			"status" => 'ok', 'isLiked' => $isLiked, "total_likes" => $total_likes, "total_comments" => $total_comments, "total_views" => $total_views
		));
	}
}
