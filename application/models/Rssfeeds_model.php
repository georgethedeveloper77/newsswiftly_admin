...
class Rssfeeds_model extends CI_Model{
    ...
    public function update_total_views($id){
      ...
      $this->db->where('id' , $id);
      $this->db->update('tbl_rss_feeds');
      $this->status = 'ok';
    }

    function getTotalViews($id){
      $this->db->select('tbl_rss_feeds.views_count');
    ...
    function getArticleData($id)
    {
      ...
      $this->db->join('tbl_rss_urls','tbl_rss_urls.id = tbl_rss_feeds.channel');
        ...
        {
          $this->db->select('tbl_rss_feeds.content');
          $this->db->from('tbl_rss_feeds');
            $this->db->where('tbl_rss_feeds.id', $id);
            $query = $this->db->get();
            $row = $query->row();
   ...
   function feedsListing($data = []){
       ...
       if(isset($data->interests) && count((array)$data->interests) > 1){
         $this->db->select('tbl_rss_feeds.*,tbl_rss_urls.source');
         $this->db->from('tbl_rss_feeds');
         $this->db->join('tbl_rss_urls','tbl_rss_urls.id = tbl_rss_feeds.channel');

         if(isset($data->sources)){
            $this->db->where_not_in('tbl_rss_feeds.channel', $data->sources);
         }
         ...
         if(isset($data->sources)){
           $sql .= " AND tbl_rss_feeds.channel  NOT IN ( '" . implode( "', '" , $data->sources ) . "')";
         }
